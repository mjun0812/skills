...
4. **裏取り**: 下書き素材から検証可能な主張を抽出し、真偽を確認する。会話で既に確認済みの事実は対象外とする。検証すべき主張が無い素材（純粋なTodoなど）ではこのステップは何もしない。
   ...
   2. **リポジトリでの確認**: Glob / Grep で存在を確認し、Read で該当箇所を読み、経緯が必要なら `git log -S` や `git blame` を使う。参照は `path:line` と commit hash で記録する。行番号は裏取りの記録用であり、本文には持ち込まない
...
7. **タイトルと本文の生成**: 下書き素材と裏取りの結果を元に、テンプレートに沿った形でタイトルと本文を生成する:
   ...
   - 並列する要素は文でなく箇条書きにし、1行1要素で書く
   - 箇条書きは指定言語に応じて以下の文体で書く
     - 日本語: 体言止めで終える。ですます調は使わない
       - GOOD: `- S3取得を共通ライブラリへ切り出し`
