...
2. **下書き素材の確保**: ユーザーの依頼にIssueの内容 (何をしたいか・何が起きているか) が含まれていれば、そのテキスト全体を「下書き素材」として使い、追加の入力は求めない。依頼に内容がまったく含まれていない場合のみ、AskUserQuestion ではなく **自由テキスト入力** で概要を受け取る:
   ...
   依頼テキストに加えて、同一会話で確認した事実 (読んだコード、コマンドの出力、エラーログ、試した案と却下した理由) も下書き素材に含める。会話の内容と依頼が矛盾する場合は依頼を優先し、矛盾はステップ9「作成前の確認」で報告する。
...
3. **関連Issueの検索**: 下書き素材の要点から2〜3語のキーワードを取り、open/closedの両方を検索する:
   ...
   - closedに同じ症状のIssueがある → `Related to #N` として扱い、ステップ9「作成前の確認」で解決済みの可能性を明示する
...
5. **種別の自動判定**: 下書き素材が何を求めているかでテンプレートを選択する。
   ...
   判定が曖昧な場合は `task` を選ぶ。判定結果はステップ11「結果の報告」でユーザーに開示する。

6. **テンプレートの読み込み**: ステップ1「前提情報の取得」で確認したテンプレート所在に従い、ステップ5「種別の自動判定」で選んだテンプレート種別のファイルを Read で読み込む。フロントマターから `labels` をデフォルトラベルとして抽出し、本文部分は構造の参考にする。
