...
5. **各コンフリクトの解消**:
   - 各コンフリクトファイルについて:
     - コンフリクトマーカー（`<<<<<<<`, `=======`, `>>>>>>>`）を表示
     - 両方のバージョンを分析:
       - **HEAD (ours)** と **theirs** が指す側は進行中の操作で入れ替わる。merge中はHEADが現在のブランチ、theirsがbase branch。rebase・cherry-pick中はHEADが適用先 (base側)、theirsが再適用中のcommit
     - 両側の変更意図を踏まえて解消を適用
     - 判断理由を説明（検出された言語で）
