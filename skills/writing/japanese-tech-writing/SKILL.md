...
日本語で説明文を書く時に、`cognitive-rhythm-writing` skillが利用可能な場合は、その規範も必ず参照する。
...
- タイトルと見出しは、原文の表記言語を維持する。意味や構成は修正してよいが、英語の語句を日本語訳またはカタカナ転写しない。役割名、識別子、固有名詞は原綴りを保持する (`reviewer`を`レビュワー`にしない)。翻訳はユーザーが明示的に求めた場合に限る。
