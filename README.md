...
The github-pr-review skill uses three reviewer agents defined in [agents/](agents/): `code-reviewer-finder`, `code-reviewer-standards`, and `code-reviewer-verifier`. Agents are distributed only through the Claude Code plugin. On other install methods the skill falls back to generic subagents given the same prompts.
